...
SOURCE_CSV = Path(os.environ.get(
    "WEB_CLUB_SOURCE_CSV",
    OUTPUT_DIR / "web_club_source_mapped_2.csv",
))
MAPPED_CSV = Path(os.environ.get(
    "WEB_CLUB_MAPPED_CSV",
    OUTPUT_DIR / "web_club_mapped_8.csv",
))
WEB_UNIVERSITY_CSV = OUTPUT_DIR / "web_university_import.csv"
...
def write_sql(rows, universities, source_to_university):
    ...
    lines = [
        ...
        "-- Review validation_status before running COMMIT or ROLLBACK.",
        "",
    ...
    lines.extend([
        "SET @actual_web_university_count = (SELECT COUNT(*) FROM web_university);",
        "SET @actual_web_club_count = (SELECT COUNT(*) FROM web_club);",
        "",
        "SELECT",
        "    @expected_web_university_count AS expected_web_university_count,",
        "    @actual_web_university_count AS actual_web_university_count,",
        "    @expected_web_club_count AS expected_web_club_count,",
        "    @actual_web_club_count AS actual_web_club_count,",
        "    CASE",
        "        WHEN @actual_web_university_count = @expected_web_university_count",
        "            AND @actual_web_club_count = @expected_web_club_count",
        "        THEN 'READY_TO_COMMIT'",
        "        ELSE 'COUNT_MISMATCH_ROLLBACK_RECOMMENDED'",
        "    END AS validation_status;",
        "",
        "-- If validation_status is READY_TO_COMMIT, run COMMIT; otherwise run ROLLBACK.",
        "-- COMMIT;",
        "-- ROLLBACK;",
        "",
        f"-- After COMMIT, rollback is available via {ROLLBACK_SQL.name}, as long as the backup tables remain.",
    ])
