...
WHERE u.provider = 'APPLE'
  ...
  AND (
    ...
    OR u.id = (
      SELECT u3.id
      FROM users u3
      WHERE u3.provider = u.provider
        AND u3.provider_id = u.provider_id
        AND u3.deleted_at IS NOT NULL
      ORDER BY u3.deleted_at DESC, u3.id DESC
      LIMIT 1
    )
  )
  AND NOT EXISTS (
  ...
  AND (
    ...
    OR NOT EXISTS (
      SELECT 1
      FROM user_oauth_account ua2
      WHERE ua2.provider = u.provider
        AND ua2.provider_id = u.provider_id
    )
  );
